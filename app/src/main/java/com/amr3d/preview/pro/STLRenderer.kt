...
class STLRenderer : GLSurfaceView.Renderer {
    ...
    private val vboIds = IntArray(4) // [0]=vertex [1]=normal [2]=wireframe
    ...
    private fun uploadModelToGPU(model: STLModel) {
        ...
        try {
            ...
            // ── رفع ألوان الرؤوس (VBO رابع) — خاص بملفات GLB اللي فيها مواد ──
            if (useVertexColors && pendingMaterials != null && pendingMaterialIndices != null) {
                val colors = buildVertexColors(model, pendingMaterials!!, pendingMaterialIndices!!)
                val cb = ByteBuffer.allocateDirect(colors.size * 4).order(ByteOrder.nativeOrder())
                cb.asFloatBuffer().put(colors); cb.position(0)
                GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, vboIds[3])
                GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, colors.size * 4, cb, GLES20.GL_STATIC_DRAW)
            }
    ...
    private fun drawSolidMesh() {
        ...
        val colorAttrHandle = GLES20.glGetAttribLocation(meshProgram, "aColor")
        ...
        val useVertexColorHandle = GLES20.glGetUniformLocation(meshProgram, "uUseVertexColor")
        ...
        if (vboReady) {
            ...
            if (useVertexColors) {
                GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, vboIds[3])
                GLES20.glEnableVertexAttribArray(colorAttrHandle)
                GLES20.glVertexAttribPointer(colorAttrHandle, 4, GLES20.GL_FLOAT, false, 0, 0)
                GLES20.glUniform1i(useVertexColorHandle, 1)
            } else {
                GLES20.glDisableVertexAttribArray(colorAttrHandle)
                GLES20.glUniform1i(useVertexColorHandle, 0)
            }
            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0)
        } else {
        ...
        if (useVertexColors) GLES20.glDisableVertexAttribArray(colorAttrHandle)
        if (vboReady) GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0)
    }
